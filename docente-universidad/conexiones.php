...
if (isset($_POST['submitFecha'])) {
    ...
    if (isset($_POST['fechaFin'])) {
        ...
        /*CONSULTA MODIFICADA */
        $consulta = "SELECT COUNT(co.id_conexion) as total, DATE_FORMAT(co.created_at,'%M %Y') as mes from conexiones as co INNER JOIN alumnos_universidad ap ON co.id_usuario = ap.id_alumno INNER JOIN docentes_universidad as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_docente = '$id_user' AND tipo = 'alumno_universidad' AND co.created_at BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
    }
} else {
    /*CONSULTA MODIFICADA */
    $consulta = "SELECT COUNT(co.id_conexion) as total, DATE_FORMAT(co.created_at,'%M %Y') as mes from conexiones as co INNER JOIN alumnos_universidad ap ON co.id_usuario = ap.id_alumno INNER JOIN docentes_universidad as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_docente = '$id_user' AND tipo = 'alumno_universidad' GROUP BY(mes) ORDER BY (mes)DESC";
}
