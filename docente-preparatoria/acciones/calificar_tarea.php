...
if (!empty($_POST)) {
    ...
    if (empty($_POST['calificacion'])) {
        ...
        //$sql_update = mysqli_query($conexion, "UPDATE grupos_preparatoria SET materia = '$materia', nombre_grupo = '$nombregrupo' , grado = '$grado' WHERE id_grupo = $idgrupo");
        ...
        $sql_update = mysqli_query($conexion, "UPDATE archivos_preparatoria SET calificacion = '$calificacion', fecha_calificado = current_timestamp(), comentario = '$comentario' WHERE id_archivo = $id_archivo");
        $alert = '<div class="alert alert-success" role="alert">¡Calificación asignada!</div>';

        //INSERTA EN DETALLE_ESTADISTICAS_preparatoria
        $query = "INSERT INTO detalle_estadisticas_preparatoria (progreso, practico, id_alumno, id_curso, id_capsula) VALUES ('2', $calificacion, '$id_alumno', $id_curso, $id_capsula)";
        $query_run = mysqli_query($conexion, $query);

        //INSERTA EN DETALLE_CAPSULAS_preparatoria
        $query1 = "INSERT INTO detalle_capsulas_preparatoria (id_capsula, id_curso, id_alumno) VALUES ($id_capsula, $id_curso, $id_alumno)";
        ...
        $insertarEstadisticas = mysqli_query($conexion, "UPDATE estadisticas_preparatoria SET trofeos = '$totalTrofeos', progreso = '$totalProgreso', puntos = '$totalPuntos', practico = '$totalPractico', teorico = '$totalTeorico' WHERE id_alumno = $id_alumno AND id_curso = '$id_curso'");
...
$sql = mysqli_query($conexion, "SELECT ar.id_archivo, ar.id_alumno, ap.nombre, ap.grado_escolar, ar.id_capsula, cp.curso, ar.id_curso, ar.calificacion, ar.comentario, DATE_FORMAT(ar.created_at,'%r %d-%m-%Y') as fecha FROM archivos_preparatoria ar 
                    JOIN alumnos_preparatoria ap
                    ON ar.id_alumno = ap.id_alumno
                    JOIN docentes_preparatoria dp
                    ON ap.id_docente = dp.id_docente
                    JOIN cursos_preparatoria cp
