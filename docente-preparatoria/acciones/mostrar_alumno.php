...
<body>
    ...
    $query2 = mysqli_query($conexion, "SELECT DISTINCT e.id_curso, c.curso
                                   ...
                                   JOIN cursos_preparatoria c ON e.id_curso = c.id_curso
                                   WHERE e.id_alumno = $idalumno");
    $cursos = mysqli_fetch_all($query2, MYSQLI_ASSOC);

    $query3 = mysqli_query($conexion, "SELECT * FROM alumnos_preparatoria WHERE id_alumno = $idalumno");
                ...
                <table id="alumnos" width="100%" class="table border-top">
                    ...
                    <tbody>
                        <tr>
                            ...
                            <?php
                            foreach ($cursos as $curso) {
                                $queryCurso = mysqli_query($conexion, "SELECT * FROM estadisticas_preparatoria WHERE id_alumno = $idalumno AND id_curso = " . $curso["id_curso"]);
