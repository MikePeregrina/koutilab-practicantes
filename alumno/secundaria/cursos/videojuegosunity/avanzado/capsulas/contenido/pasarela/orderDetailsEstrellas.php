...
if (!empty($_GET['id_alumno']) && !empty($_GET['id_capsula']) && !empty($_GET['id_curso']) && !empty($_GET['payment_amount'])) {
    ...
    if ($result_estrellas->num_rows > 0) {
        ...
        if ($cantidad_estrellas >= 150) {
            ...
            $query_update_estrellas = mysqli_query($conexion, "UPDATE total_estrellas_secundaria SET estrellas = '$nueva_cantidad_estrellas' WHERE id_alumno = '$id_user'");

            // Insertar en detalle_capsulas_pago_secundaria
            $query_insert_permiso = mysqli_query($conexion, "INSERT INTO detalle_capsulas_pago_secundaria(id_capsula, id_alumno, id_curso) VALUES ('$id_capsula', '$id_alumno', '$id_curso')");
