...
$permiso_intento = 10;
$sql_permisos = mysqli_query($conexion, "SELECT * FROM detalle_capsulas_universidaddad WHERE id_capsula = $permiso_intento AND id_alumno = '$id_user' AND id_curso = 3");
$result_sql_permisos = mysqli_num_rows($sql_permisos);
//Script para poder ver cuantos intentos lleva el alumno en la capsula y mostrar cuantos puntos gano dependiendo los intentos

//Contar total de intentos
$consultaIntentos = mysqli_query($conexion, "SELECT intentos FROM detalle_intentos_universidaddad WHERE id_capsula = $permiso_intento AND id_alumno = $id_user AND id_curso = 3");
    ...
    <div class="contenido">
        ...
        <div class="container">
            <section><!--GENERANDO SECCION PARA PREGUNTAS Y RESPUESTAS-->
                ...
                <h3>
                    1. Para llamar la atención de los usuarios, nosotros como protgramadores usamos un
                    <select class="select" id="respuesta0"><!--Generando lista de opciones de respuesta de la pregunta 1-->
                        <option value="----">...</option>
                        <option value="correcto">Carrusel</option>
                        <option value="incorrecto">Caballo</option>
                        <option value="incorrecto">Contenedor</option>
                    </select>
                    que contiene texto o imágenes en movimiento
                </h3> <br>
                <!--Generando pregunta 2-->
                <h3>2. Para crear un carrusel de imágenes, podemos ocupar un
                    <select class="select" id="respuesta1"><!--Generando lista de opciones de respuesta de la pregunta 2-->
                        <option value="----">...</option>
                        <option value="incorrecto">Carrusel</option>
                        <option value="correcto">Slider</option>
                        <option value="incorrecto">Div</option>
                    </select>
                </h3> <br>
                <!--Generando pregunta 3-->
                <h3>3. El crear un carrusel de imágenes nos ayuda a que nuestros sitios web sean más
                    <select class="select" id="respuesta2"><!--Generando lista de opciones de respuesta de la pregunta 3-->
                        <option value="----">...</option>
                        <option value="correcto">Llamativos</option>
                        <option value="incorrecto">Aburridos</option>
                        <option value="incorrecto">Estresantes</option>
                    </select>
                </h3> <br><!--Generando primer pregunta-->

                <h3>4. En programación, podemos crear un carrusel de imágenes con un
                    <select class="select" id="respuesta3"><!--Generando opciones de respuesta de la pregunta-->
                        <option value="----">...</option>
                        <option value="correcto">slider-container</option>
                        <option value="incorrecto">carrousel-container</option>
                        <option value="incorrecto">info-container</option>
                    </select>
                    que contenga toda nuestra información
    ...
    <script>
        ...
        function iniciarTiempo() {
            ...
            if (segundos == 0) {
                ...
                var param = "score=" + 0 + "&validar=" + 'incorrecto' + "&permiso=" + 10 + "&id_curso=" + 3; //cancatenation
        ...
        function alertExcelent() {
            ...
            var xmlhttp = new XMLHttpRequest();
            var param = "score=" + 10 + "&validar=" + 'correcto' + "&permiso=" + 10 + "&id_curso=" + 3; //cancatenation
            xmlhttp.open("POST", "../../acciones/insertar_pd10.php", true);
            xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xmlhttp.send(param);
