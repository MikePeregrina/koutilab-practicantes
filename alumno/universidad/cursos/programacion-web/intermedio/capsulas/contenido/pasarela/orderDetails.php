...
if (!empty($_GET['payment_id']) && !empty($_GET['item_number']) && !empty($_GET['item_name']) && !empty($_GET['payment_amount']) && !empty($_GET['payment_currency'])) {
	...
	$query_insert = mysqli_query($conexion, "INSERT INTO payment_universidaddad(payment_id, item_number, item_name, payment_amount, payment_currency, create_at, id_alumno) VALUES ('$payment_id', '$item_number', '$item_name', '$payment_amount', '$payment_currency', current_timestamp(), '$id_alumno')");
