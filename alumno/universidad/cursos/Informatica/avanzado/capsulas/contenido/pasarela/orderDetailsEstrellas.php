...
if (!empty($_GET['id_alumno']) && !empty($_GET['id_capsula']) && !empty($_GET['id_curso']) && !empty($_GET['payment_amount'])) {
    ...
    if ($result_estrellas->num_rows > 0) {
        ...
        if ($cantidad_estrellas >= 50) {
            ...
            $query_update_estrellas = mysqli_query($conexion, "UPDATE total_estrellas_universidad SET estrellas = '$nueva_cantidad_estrellas' WHERE id_alumno = '$id_user'");

            // Insertar en detalle_capsulas_pago_universidad
