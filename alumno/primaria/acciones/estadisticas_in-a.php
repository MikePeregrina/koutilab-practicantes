...
/*function actualizarEstrellas($id_user, $conexion)
{
    ...
    $sql = "SELECT conexiones FROM alumnos_primaria WHERE id_alumno = $id_user";
    $result = $conexion->query($sql);

    if ($result->num_rows > 0) {
        $row = $result->fetch_assoc();
        $total_conexiones = $row['conexiones'];
        //Verificar estrellas en total_estrellas_primaria
        $sql = mysqli_query($conexion, "SELECT * FROM total_estrellas_primaria WHERE id_alumno = '$id_user'");
        $result_sql = mysqli_num_rows($sql);

        if ($result_sql == 0) {
            $sql =  mysqli_query($conexion, "INSERT INTO total_estrellas_primaria (id_alumno, estrellas) VALUES ($id_user, 1)");
        }

        // Si es múltiplo de 20 o es la primera conexión, insertar una estrella en la tabla total_estrellas_primaria
        if ($total_conexiones % 20 == 0) {
            // Si no es múltiplo de 20, actualizar el campo estrellas en la tabla total_estrellas_primaria
            $sql =  mysqli_query($conexion, "UPDATE total_estrellas_primaria SET estrellas = estrellas + 1 WHERE id_alumno = $id_user");
        } else {
            // Si no es múltiplo de 20, actualizar el campo estrellas en la tabla total_estrellas_primaria
            $sql =  mysqli_query($conexion, "UPDATE total_estrellas_primaria SET estrellas = estrellas + 0 WHERE id_alumno = $id_user");
        }
    }
}
...
$consultaEstadistica = mysqli_query($conexion, "SELECT trofeos, SUM(trofeos) AS total_trofeos, progreso, SUM(progreso) AS total_progreso, puntos, SUM(puntos) AS total_puntos, practico, SUM(practico) AS total_practico, teorico, SUM(teorico) AS total_teorico FROM estadisticas_primaria WHERE id_alumno = $id_user AND id_curso = 9"); // Modificando consulta por cada curso
...
$sql_verificar_rutas = mysqli_query($conexion, "SELECT a.* FROM acceso_cursos_primaria a WHERE a.id_alumno = $id_user");
            ...
            <div class="board p-2">
                <table id="alumnos" width="100%" class="table border-top" style="z-index: 1;">
                    ...
                    <tbody>
                        ...
                        $query_alumnos = mysqli_query($conexion, "SELECT *FROM detalle_estadisticas_primaria
                    WHERE id_curso = 9");
