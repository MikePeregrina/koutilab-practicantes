...
<body onload="iniciarTiempo()">
    ...
    <div class="titulo-gen">
        <h2 class="titulo"><b>VARIABLES GLOBALES</b></h2>
    </div>
    ...
    <div class="contenido">
        ...
        <div class="container">
            <section><!--GENERANDO SECCION PARA PREGUNTAS Y RESPUESTAS-->
                ...
                <h3>1- ¿Qué tipo de variable podemos ocupar si queremos hacer que esté disponible para todo el código que tenemos?
                    <select class="select" id="respuesta0"><!--Generando lista de opciones de respuesta de la pregunta 1-->
                        <option value="----">...</option>
                        <option value="incorrecto">Variable local</option>
                        <option value="incorrecto">Variable privada</option>
                        <option value="correcto">Variable global</option>
                    </select>
                </h3>
                <!--Generando pregunta 2-->
                <h3>2- ¿Qué tipo de sintáxis ocupamos para definir una variable global?
                    <select class="select" id="respuesta1"><!--Generando lista de opciones de respuesta de la pregunta 2-->
                        <option value="----">...</option>
                        <option value="correcto">saludar = 'Hola mundo'</option>
                        <option value="incorrecto">def saludar(): saludar = 'Hola mundo'</option>
                    </select>
                </h3>
                <!--Generando pregunta 3-->
                <h3>3- Ahora, ¿qué sintáxis ocuparíamos para crear una variable local?
                    <select class="select" id="respuesta2"><!--Generando lista de opciones de respuesta de la pregunta 3-->
                        <option value="----">...</option>
                        <option value="incorrecto">saludar = 'Hola mundo'</option>
                        <option value="correcto">def saludar(): saludar = 'Hola mundo'</option>
                    </select>
                </h3><!--Generando primer pregunta-->

                <h3>4- Un erros común en las variables globales son la...
                    <select class="select" id="respuesta3"><!--Generando opciones de respuesta de la pregunta-->
                        <option value="----">...</option>
                        <option value="correcto">Sobreescritura</option>
                        <option value="incorrecto">Sobre peso</option>
                    </select>
                </h3><!--Generando primer pregunta-->
            </section>
        </div>
    ...
    <script>
        ...
        function iniciarTiempo() {
            ...
            if (segundos == 0) {
                ...
                var param = "score=" + 0 + "&validar=" + 'incorrecto' + "&permiso=" + 13 + "&id_curso=" + 5 + "&redireccion=" + '../contenido/juegos/cjp1-4.php'; //cancatenation
        ...
        function alertExcelent() {
            ...
            var xmlhttp = new XMLHttpRequest();
            var param = "score=" + 10 + "&validar=" + 'correcto' + "&permiso=" + 13 + "&id_curso=" + 5 + "&redireccion=" + '../contenido/juegos/cjp1-4.php'; //cancatenation
            xmlhttp.open("POST", "../../acciones/insertar_juego.php", true);
            xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
            xmlhttp.send(param);
            correcto.play();
