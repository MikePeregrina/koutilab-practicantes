...
<body onload="iniciarTiempo()">
  ...
  <div class="titulo-gen">
    <h2 class="titulo"><b>JUEGO INFINITO</b></h2>
  </div>
  ...
  <div class="contenido">
    ...
    <div class="container">
      <section>
        ...
        <h3>
          1- ¿Qué archivo actuará como generador de niveles?
          <select class="select" id="respuesta0">
            <!--Generando lista de opciones de respuesta de la pregunta 1-->
            <option value="----">...</option>
            <option value="incorrecto">Script</option>
            <option value="incorrecto">JSON</option>
            <option value="correcto">LevelGenerator</option>
          </select>
        </h3>
        <!--Generando pregunta 2-->
        <h3>
          2- ¿Cuáles son los elementos que se usan para generar contenido aleatorio?
          <select class="select" id="respuesta1">
            <!--Generando lista de opciones de respuesta de la pregunta 2-->
            <option value="----">...</option>
            <option value="incorrecto">Algoritmos</option>
            <option value="incorrecto">CSS y PHP</option>
            <option value="correcto">Scripts y Algoritmos</option>
          </select>
        </h3>
        <!--Generando pregunta 3-->
        <h3>
          3- ¿Cuál es el nombre del arreglo donde se almacenan los archivos prefabricados?
          <select class="select" id="respuesta2">
            <!--Generando lista de opciones de respuesta de la pregunta 3-->
            <option value="----">...</option>
            <option value="correcto">levelPrefabs</option>
            <option value="incorrecto">Collider</option>
            <option value="incorrecto">LevelGenerator</option>
          </select>
        </h3>
        <!--Generando primer pregunta-->

        <h3>
          4- ¿Qué componente debe de contener los prefabricados de nivel?
          <select class="select" id="respuesta3">
            <!--Generando opciones de respuesta de la pregunta-->
            <option value="----">...</option>
            <option value="incorrecto">spawnDistance</option>
            <option value="incorrecto">despawnDistance</option>
            <option value="correcto">Collider</option>
          </select>
        </h3>
        <!--Generando primer pregunta-->
      </section>
  ...
  <script>
    ...
    function iniciarTiempo() {
      ...
      if (segundos == 0) {
        ...
        var param = "score=" + 0 + "&validar=" + 'incorrecto' + "&permiso=" + 32 + "&id_curso=" + 11; //cancatenation
    ...
    function alertExcelent() {
      var xmlhttp = new XMLHttpRequest();
      var param = "score=" + 10 + "&validar=" + 'correcto' + "&permiso=" + 32 +"&id_curso=" + 11; //cancatenation
      xmlhttp.open("POST", "../../acciones/insertar_pd32.php", true);
      xmlhttp.setRequestHeader("Content-Type", "application/x-www-form-urlencoded");
      xmlhttp.send(param);
      Swal.fire({
