...
function selectWord() {
  ...
  if (
    palabraseleccionada.id !== "valores" &&
    palabraseleccionada.id !== "inversion" &&
    palabraseleccionada.id !== "expreciones" &&
    palabraseleccionada.id !== "verdadero" &&
    palabraseleccionada.id !== "comparar"
  ) {
...
function checkAnswer(respuesta) {
  ...
  if (palabraseleccionada) {
    ...
    if (respuesta === "comparar" && idPalabraSeleccionada === "operadores") {
      ...
      contexto.lineTo(550, 310);
      contexto.stroke(); // "Guardar" cambios
      //sumamos al contador
      respuestasCorrectas++;
    } else if (respuesta === "verdadero" && idPalabraSeleccionada === "and") {
      palabraseleccionada.classList.add("correcto");
      contexto.beginPath();
      contexto.lineWidth = 3;
      contexto.strokeStyle = "#84c42c";
      contexto.moveTo(0, 85);
      contexto.lineTo(470, 210);
      contexto.stroke();
      respuestasCorrectas++;
    } else if (respuesta === "valores" && idPalabraSeleccionada === "or") {
      palabraseleccionada.classList.add("correcto");
      contexto.beginPath();
      contexto.lineWidth = 3;
      contexto.strokeStyle = "#84c42c";
      contexto.moveTo(0, 150);
      contexto.lineTo(470, 20);
      contexto.stroke();
      respuestasCorrectas++;
    } else if (respuesta === "inversion" && idPalabraSeleccionada === "not") {
      palabraseleccionada.classList.add("correcto");
      contexto.beginPath();
      contexto.lineWidth = 3;
      contexto.strokeStyle = "#84c42c";
      contexto.moveTo(0, 215);
      contexto.lineTo(470, 80);
      contexto.stroke();
      respuestasCorrectas++;
    } else if (
      respuesta === "expreciones" &&
      idPalabraSeleccionada === "comparaciones"
    ) {
      palabraseleccionada.classList.add("correcto");
      contexto.beginPath();
      contexto.lineWidth = 3;
