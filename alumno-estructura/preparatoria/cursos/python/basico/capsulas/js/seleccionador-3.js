...
function selectWord() {
    ...
    if (
        ...
        palabraseleccionada.id !== 'corchetes' &&
        palabraseleccionada.id !== 'nones'
    ) {
...
function checkAnswer(respuesta) {
    ...
    if (palabraseleccionada) {
        ...
        if (respuesta === 'datos' && idPalabraSeleccionada === 'estructura') {
            ...
            contexto.lineTo(550, 25);
            contexto.stroke(); // "Guardar" cambios
            //sumamos al contador
            respuestasCorrectas++;
        } else if (respuesta === 'valor' && idPalabraSeleccionada === 'clave') {
            palabraseleccionada.classList.add('correcto');
            contexto.beginPath();
            contexto.lineWidth = 3;
            contexto.strokeStyle = "#84c42c";
            contexto.moveTo(0, 85);
            contexto.lineTo(550, 140);
            contexto.stroke();
            respuestasCorrectas++;
        }
        else if (
            respuesta === 'informacion' && idPalabraSeleccionada === 'almacena'
        ) {
            palabraseleccionada.classList.add('correcto');
            contexto.beginPath();
            contexto.lineWidth = 3;
            ...
            respuesta === 'corchetes' && idPalabraSeleccionada === 'sintaxis'
        ) {
            palabraseleccionada.classList.add('correcto');
            contexto.beginPath();
            contexto.lineWidth = 3;
            contexto.strokeStyle = "#84c42c";
            contexto.moveTo(0, 190);
            contexto.lineTo(550, 190);
            contexto.stroke();
            respuestasCorrectas++;


        } else if (
            respuesta === 'none' && idPalabraSeleccionada === 'defecto'
        ) {
            palabraseleccionada.classList.add('correcto');
            contexto.beginPath();
            contexto.lineWidth = 3;
