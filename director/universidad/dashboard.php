...
if (isset($_POST['submitFecha'])) {
  ...
  if (isset($_POST['fechaFin'])) {
    ...
    $consulta = "SELECT SUM(payment_amount * $calculo_ganancias) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_universidad as pp INNER JOIN alumnos_universidad as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_universidad as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' AND create_at BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
  }
} else {

  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT SUM(payment_amount * $calculo_ganancias) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_universidad as pp INNER JOIN alumnos_universidad as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_universidad as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
...
<body>
    ...
    <div class="right-content">
      ...
      <div class="latd">
        <div class="tabla-ingr">
          ...
          FROM
            payment_universidad AS pp
          INNER JOIN
            alumnos_universidad AS ap ON pp.id_alumno = ap.id_alumno
          INNER JOIN
            directores_universidad AS dp ON ap.id_escuela = dp.id_escuela
          WHERE
  ...
  if (isset($_FILES["image"]["name"])) { /*Si el archivo existe */
    ...
    if (!in_array($imageExtension, $validImageExtension)) {
      ...
      $query = "UPDATE directores_universidad SET image = '$newImageName' WHERE id_director = $id"; //Actualizando imagen en BD
