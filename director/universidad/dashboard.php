...
if (isset($_POST['submitFecha'])) {
  ...
  if (isset($_POST['fechaFin'])) {
    ...
    $consulta = "SELECT SUM(payment_amount * 0.1) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_universidad as pp INNER JOIN alumnos_universidad as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_universidad as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' AND create_at BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
  }
} else {

  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT SUM(payment_amount * 0.1) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_universidad as pp INNER JOIN alumnos_universidad as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_universidad as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
