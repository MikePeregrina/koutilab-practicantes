...
if (isset($_POST['submitFecha'])) {
  ...
  if (isset($_POST['fechaFin'])) {
    ...
    $consulta = "SELECT SUM(payment_amount * $calculo_ganancias) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_preparatoria as pp INNER JOIN alumnos_preparatoria as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_preparatoria as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' AND create_at BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
  }
} else {

  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT SUM(payment_amount * $calculo_ganancias) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_preparatoria as pp INNER JOIN alumnos_preparatoria as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_preparatoria as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
...
<body>
    ...
    <div class="right-content">
      ...
      <div class="latd">
        <div class="tabla-ingr">
          ...
          FROM
            payment_preparatoria AS pp
          INNER JOIN
            alumnos_preparatoria AS ap ON pp.id_alumno = ap.id_alumno
          INNER JOIN
            directores_preparatoria AS dp ON ap.id_escuela = dp.id_escuela
          WHERE
  ...
  if (isset($_POST['enviarcontrasena'])) {
    $iddirector = $_SESSION['id_director_preparatoria'];
    $contrasena = md5($_POST['contrasena']);

    $sql_update = mysqli_query($conexion, "UPDATE directores_preparatoria SET contrasena = '$contrasena' WHERE id_director = '$iddirector'");
