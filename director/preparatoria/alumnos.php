...
if (isset($_POST['submitFecha'])) {
  ...
  $consulta = "SELECT SUM(conexiones) as total, DATE_FORMAT(fecha_registro,'%M %Y') as mes from alumnos_preparatoria as pp INNER JOIN directores_preparatoria as dp ON pp.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
