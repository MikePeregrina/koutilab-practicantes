...
if (isset($_POST['submitFecha'])) {
  ...
  if (isset($_POST['fechaFin'])) {
    ...
    $consulta = "SELECT SUM(pp.conexiones) as total, DATE_FORMAT(pp.fecha_registro,'%M %Y') as mes from alumnos_preparatoria as pp INNER JOIN directores_preparatoria as dp ON pp.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' AND pp.fecha_registro BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
  }
} else {

  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT SUM(pp.conexiones) AS total, DATE_FORMAT(pp.fecha_registro, '%M %Y') AS mes
  FROM alumnos_preparatoria AS pp
  INNER JOIN directores_preparatoria AS dp ON pp.id_escuela = dp.id_escuela
  WHERE dp.id_director = $id_user
  GROUP BY mes
  ORDER BY mes DESC;
  ";
}
