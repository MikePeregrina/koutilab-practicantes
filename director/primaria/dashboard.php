...
$user = mysqli_fetch_assoc(mysqli_query($conexion, "SELECT * FROM directores_primaria a JOIN escuelas e ON a.id_escuela = e.id_escuela WHERE id_director = $id_user"));
...
if (isset($_POST['submitFecha'])) {
  ...
  if (isset($_POST['fechaFin'])) {
    ...
    $consulta = "SELECT SUM(payment_amount * $calculo_ganancias) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_primaria as pp INNER JOIN alumnos_primaria as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_primaria as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' AND create_at BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
  }
} else {

  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT SUM(payment_amount * $calculo_ganancias) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_primaria as pp INNER JOIN alumnos_primaria as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_primaria as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
...
<body>
  ...
  <section>
    ...
    <div class="container-info">
      <?php
      $data2 = mysqli_query($conexion, "SELECT * FROM directores_primaria d INNER JOIN escuelas e WHERE d.id_escuela = e.id_escuela AND id_director = '$id_user'");
      while ($consulta = mysqli_fetch_array($data2)) {
        ...
        echo "  <h3 class='info-heading'>CCT: <span>" . $consulta['nombre'] . "</span></h3>";
      }
      ?>
  ...
  if (isset($_FILES["image"]["name"])) { /*Si el archivo existe */
    ...
    if (!in_array($imageExtension, $validImageExtension)) {
      ...
      $query = "UPDATE directores_primaria SET image = '$newImageName' WHERE id_director = $id"; //Actualizando imagen en BD
  ...
  if (isset($_POST['enviarcontrasena'])) {
    $iddirector = $_SESSION['id_director_primaria'];
    $contrasena = md5($_POST['contrasena']);

    $sql_update = mysqli_query($conexion, "UPDATE directores_primaria SET contrasena = '$contrasena' WHERE id_director = '$iddirector'");
