...
if (isset($_POST['submitFecha'])) {
  ...
  $consulta = "SELECT SUM(payment_amount * 0.1) as total, DATE_FORMAT(create_at,'%M %Y') as mes from payment_primaria as pp INNER JOIN alumnos_primaria as ap ON pp.id_alumno = ap.id_alumno INNER JOIN directores_primaria as dp ON ap.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
