...
if (isset($_POST['submitFecha'])) {
  ...
  if (isset($_POST['fechaFin'])) {
    ...
    $consulta = "SELECT COUNT(id_conexion) as total, DATE_FORMAT(created_at,'%M %Y') as mes from conexiones as co INNER JOIN docentes_secundaria as ap ON co.id_usuario = ap.id_docente INNER JOIN directores_secundaria as dp ON ap.id_escuela = dp.id_escuela WHERE tipo = 'docente_secundaria' AND dp.id_director = '$id_user' AND created_at BETWEEN '$fechaInicio' and '$fechaFin' GROUP BY(mes) ORDER BY (mes)DESC";
  }
} else {

  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT COUNT(id_conexion) as total, DATE_FORMAT(created_at,'%M %Y') as mes from conexiones as co INNER JOIN docentes_secundaria as ap ON co.id_usuario = ap.id_docente INNER JOIN directores_secundaria as dp ON ap.id_escuela = dp.id_escuela WHERE tipo = 'docente_secundaria' AND dp.id_director = '$id_user' GROUP BY(mes) ORDER BY (mes)DESC";
}
...
function actualizarGrafica()
{
  ...
  // Consulta para obtener los datos de ganancias
  $consulta = "SELECT SUM(conexiones) as total, DATE_FORMAT(fecha_registro,'%M %Y') as mes from docentes_secundaria as pp INNER JOIN directores_secundaria as dp ON pp.id_escuela = dp.id_escuela WHERE dp.id_director = '$id_user' AND fecha_registro BETWEEN '.$fechaInicio.' and '.$fechaFin.' GROUP BY(mes) ORDER BY (mes)DESC";
  // Ejecutar la consulta
  $resultado = $conexion->query($consulta);
